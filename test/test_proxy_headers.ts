...
const AVAILABLE_TESTS: Record<string, TestFn> = {
  ...
  async ky(config) {
    try {
      ...
      if (headerValue) return new TestResult("ky", true, headerValue, null, response.status);
      return new TestResult(
        "ky",
        false,
        null,
      ...
      return new TestResult("ky", false, null, (err as Error).message);
    }
  },
  ...
  async wretch(config) {
    try {
      ...
      if (headerValue) return new TestResult("wretch", true, headerValue, null, response.status);
      return new TestResult(
        "wretch",
        false,
        null,
  ...
  async "make-fetch-happen"(config) {
    try {
      ...
      if (headerValue) return new TestResult("make-fetch-happen", true, headerValue, null, response.status);
      return new TestResult(
        "make-fetch-happen",
        false,
        null,
      ...
      return new TestResult("make-fetch-happen", false, null, (err as Error).message);
    }
  },
  ...
  async needle(config) {
    try {
      ...
      return new TestResult("needle", false, null, (err as Error).message);
    }
  },
